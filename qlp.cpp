...
#include "src/ui/llmoptionspage.h"

using namespace Core;

namespace QLP::Internal {

static LLMOptionsPage *optionsPage;

class QLPPlugin final : public ExtensionSystem::IPlugin
{
    Q_OBJECT
    Q_PLUGIN_METADATA(IID "org.qt-project.Qt.QtCreatorPlugin" FILE "QLP.json")
...
public:
    ...
    void initialize() final
    {
        ...
        menu->menu()->setTitle(Tr::tr("QLP"));
        ActionManager::actionContainer(Core::Constants::M_TOOLS)->addMenu(menu);

        ActionBuilder(this, Constants::ACTION_ID)
            .addToContainer(Constants::MENU_ID)
            .setText(Tr::tr("QLP Action"))
            .setDefaultKeySequence(Tr::tr("Ctrl+Alt+Meta+A"))
            .addOnTriggered(this, &QLPPlugin::triggerAction);

        auto mainWindow = Core::ICore::mainWindow();

        dockWidget = new ChatDockWidget(mainWindow);
        mainWindow->addDockWidget(Qt::RightDockWidgetArea, dockWidget);

        optionsPage = new LLMOptionsPage;
...
private:
    void triggerAction()
    {
        QMessageBox::information(ICore::dialogParent(),
                                 ...
                                 Tr::tr("This is an action from QLP."));
    }

private:
    ChatDockWidget *dockWidget = nullptr;
};
